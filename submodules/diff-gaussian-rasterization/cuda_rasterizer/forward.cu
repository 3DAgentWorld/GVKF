...
regressCUDA(
	...
	const float* __restrict__ bg_color,
	float* __restrict__ out_color,
	float* __restrict__ regress_sdf,
	float* __restrict__ out_color_integrated)
{
	...
	if (DEBUG_REGRESS && PRINT_REGRESS_INFO){
		if (pix.x == 0 && pix.y == 0){
			printf("in regressCUDA, pixf is %.0f %.0f focal_x_y: %.2f %.2f g_toDo: %d p_toDo: %d\n", pixf.x, pixf.y, focal_x, focal_y, toDo, p_toDo);
		}
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int k = 0; k < 5; ++k){
				...
				// GOF's method to calculate ray-gs intersection, smart
				double AA = ray_local_scaled.x * ray_local_scaled.x + ray_local_scaled.y * ray_local_scaled.y + ray_local_scaled.z * ray_local_scaled.z;
				double BB = 2 * (ray_local_scaled.x * cam_pos_local_scaled.x + ray_local_scaled.y * cam_pos_local_scaled.y + ray_local_scaled.z * cam_pos_local_scaled.z);
				double CC = cam_pos_local_scaled.x * cam_pos_local_scaled.x + cam_pos_local_scaled.y * cam_pos_local_scaled.y + cam_pos_local_scaled.z * cam_pos_local_scaled.z;
	...
	while (true){
		...
		for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
		{
			...
			for (int j = 0; !second_done && j < min(BLOCK_SIZE, toDo); j++)
			{
				...
				for (int k = 0; k < num_projected; k++){
					...
					// GOF's way to calculate intersection, smart
					double AA = ray_local_scaled.x * ray_local_scaled.x + ray_local_scaled.y * ray_local_scaled.y + ray_local_scaled.z * ray_local_scaled.z;
					double BB = 2 * (ray_local_scaled.x * cam_pos_local_scaled.x + ray_local_scaled.y * cam_pos_local_scaled.y + ray_local_scaled.z * cam_pos_local_scaled.z);
					double CC = cam_pos_local_scaled.x * cam_pos_local_scaled.x + cam_pos_local_scaled.y * cam_pos_local_scaled.y + cam_pos_local_scaled.z * cam_pos_local_scaled.z;
					...
					// nonlinear
					// float tmp_alpha = 1.0f / point_alphas[k] - 1.0f;
					// tmp_alpha = std::log(tmp_alpha);
					// simulate inverse curve, to avoid solving equation, u=0.15 usually better
					// however, gof's refine has better results than this way
					// tmp_alpha = std::clamp(tmp_alpha, -10.0f, 10.0f);
					// point_alphas[k] = tmp_alpha-0.15f;
		...
		if (inside)
		{
			// write alphas to sdf
			for (int k = 0; k < num_projected; k++){
				// regress_sdf[projected_ids[k]] = point_alphas[k];
				// regress_sdf[projected_ids[k]] = std::log(1.0f / point_alphas[k] - 1.0f)-0.15f;
				float alpha = point_alphas[k]; 
				if (alpha >=1.0f){
					regress_sdf[projected_ids[k]] = -20.0f;
				}else if (alpha <=0.0f){
					regress_sdf[projected_ids[k]] = 20.0f;
				}else{
					alpha = 1.0f / alpha - 1.0f;
					regress_sdf[projected_ids[k]] = std::log(alpha) - 0.15f;
				}
				// float alpha = point_alphas[k]; 
				// if (alpha >=1.0f){
				// 	regress_sdf[projected_ids[k]] = -20.0f;
				...
				// 		alpha = 200.0f;
				// 	}else{
				// 		alpha = 1.0f / (alpha+0.000001f) - 1.0f;
				// 	}
...
{
	regressCUDA<NUM_CHANNELS> << <grid, block >> > (
		gaussian_ranges,
		point_ranges,
		gaussian_list,
		point_list,
