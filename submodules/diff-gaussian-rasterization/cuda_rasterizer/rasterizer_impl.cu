...
int CudaRasterizer::Rasterizer::regress(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	...
	bool debug)
{
	if (DEBUG_REGRESS && PRINT_REGRESS_INFO){
		printf("in CudaRasterizer::Rasterizer::regress\n");
	...
	if (DEBUG_REGRESS && PRINT_REGRESS_INFO){
		printf("==in CudaRasterizer::Rasterizer::regress, P: %d num_rendered: %d binning_chunk_size: %d\n", P, num_rendered, binning_chunk_size);
		// for (int i = 0; i < 100; i++){
	...
	if (DEBUG_REGRESS && PRINT_REGRESS_INFO){
		printf("in CudaRasterizer::Rasterizer::regress, PN: %d num_integrated: %d point_binning_chunk_size: %d\n", PN, num_integrated, point_binning_chunk_size);
		// for (int i = 0; i < 100; i++){
	...
	if (DEBUG_REGRESS && PRINT_REGRESS_INFO){
		printf("in CudaRasterizer::Rasterizer::regress, after my sorting num_integrated: %d point_binning_chunk_size: %d\n", num_integrated, point_binning_chunk_size);
	}
