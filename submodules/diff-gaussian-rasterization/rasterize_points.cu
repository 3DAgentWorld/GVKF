...
torch::Tensor
RegressGaussiansCUDA(
	const torch::Tensor& background,
	const torch::Tensor& querys,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	const torch::Tensor& scales,
	...
	const bool debug)
{
  ...
  if (querys.ndimension() != 2 || querys.size(1) != 3) {
    AT_ERROR("querys must have dimensions (num_points, 3)");
  }

  const int PN = querys.size(0);
  const int P = means3D.size(0);
  const int H = image_height;
  const int W = image_width;
  ...
  torch::Tensor out_color = torch::full({OUTPUT_CHANNELS, H, W}, 0.0, float_opts);
  torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor sdf_regress = torch::full({PN}, -19.0f, float_opts);
  ...
  if(P != 0 && PN != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::regress(
	    geomFunc,
		binningFunc,
		imgFunc,
		pointFunc,
		...
		out_color.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		sdf_regress.contiguous().data<float>(),
